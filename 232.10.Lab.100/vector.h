...
namespace custom {
   ...
   public:
     ...
     ~vector();
      ...
      void swap(vector& rhs) {
         // buffer
         T * tmpData = this->data;
         size_t tmpSize = this->numElements;
         size_t tmpCapacity = this->numCapacity;
         ...
         // rhs
         rhs.data = tmpData;
         rhs.numElements = tmpSize;
         rhs.numCapacity = tmpCapacity;
      }
   ...
   ****************************************/
   ...
   vector <T> ::vector(size_t num) {
      ...
      // exception: log2(0) returns a number approaching either 0 or ?
      if (!num) {
         data = nullptr;
         numCapacity = 0;
         numElements = 0;
   ...
   ****************************************/
   ...
   vector <T> ::vector(const vector& rhs) {
      ...
      this->data = nullptr;
      this->numElements = rhs.numElements;
      this->numCapacity = rhs.numElements; // don't allocate more than we need to
   ...
   ****************************************/
   ...
   vector <T> ::vector(vector&& rhs) {
      // `vector<T> vLhs(std::move(vRhs))`

      // tag pointer and info from rhs to this
      data = rhs.data;
      numElements = rhs.numElements;
      numCapacity = rhs.numCapacity;

      // std::move 'steals' the rhs data
      rhs.data = nullptr;
      rhs.numElements = 0;
      rhs.numCapacity = 0;
   }
   ...
   ****************************************/
   ...
   T& vector <T> ::at(size_t index) {
      if (index > numCapacity) throw std::out_of_range("index > size()");
      if (empty()) throw std::out_of_range("size() == 0");
   ...
   **************************************/
   ...
   void vector <T> ::push_back(const T& t) {
      ...
      while (numCapacity <= numElements) {
         ...
         for (int i = 0; i < numCapacity; i++)  // transfer elements from tmp to data (if they exist)
               data[i] = tmp[i];
      }

      // now that we know we have enough space for it, push new variable
      data[size()] = t;
      numElements++;
   }
